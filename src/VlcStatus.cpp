...
private:
	...
	QString m_title, m_artist, m_album, m_description, m_album_path;
};
...
/** Get the description of the track playing. */
QString VlcStatus::description() const {
	QMutexLocker lock(&m_mutex);
	return m_description;
}
...
void VlcStatus::process_reply(QNetworkReply* reply) {
	...
	if(xml_reader.parse(network_source)) {
		...
		//QMutexLocker lock(&m_mutex);
		{
			...
			m_description = handler.description();
